...
# Ниже идёт полностью рабочая версия
...
# Выше идёт поностью рабочая версия
...
def get_all_links(html):
    ...
    for td in tools:
        a = td.find('a').get('href')
        altsnab_link = 'http://altaysnab.ru' + a
        tools_links.append(altsnab_link)
    return tools_links
...
def write_final_links(final_link):
    with open('links.csv', 'w') as f:
        writer = csv.writer(f)
        for link in final_link:
            writer.writerow([link])
...
def main():
    url = 'http://altaysnab.ru/catalog/'
    all_links = get_all_links(get_html(url))
    final_link = get_final_links(all_links)
    write_final_links(final_link)
